...
function MatchTab({ totalCount }) {
  ...
  return (
    <div>
      ...
      {shortlist.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 18px', borderRadius: 10, background: 'color-mix(in srgb, var(--brand) 8%, transparent)', border: '1px solid var(--border)', marginBottom: 16 }}>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 16 }}>
        {onShortlist && (
          <button onClick={onShortlist} aria-label="Shortlist"
            style={{ display: 'grid', placeItems: 'center', color: shortlisted ? '#F5A623' : 'var(--text-faint)' }}>
